...
if [ -f .env ]; then
    sed -i "s#^DATABASE_URL=.*#DATABASE_URL=\"postgresql://user:password@127.0.0.1:${PG_HOST_PORT}/${PROJECT_NAME}?serverVersion=17\&charset=utf8\"#" .env
else
    echo "DATABASE_URL=\"postgresql://user:password@127.0.0.1:${PG_HOST_PORT}/${PROJECT_NAME}?serverVersion=17\&charset=utf8\"" > .env
fi
