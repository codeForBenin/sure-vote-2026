...
export default class extends Controller {
    ...
    renderEligibilityReport(data, container) {
        ...
        const isEligible = global.is_eligible_everywhere; // condition stricte "chaque circo" selon demande user

        let html = `
            <div class="p-8 bg-slate-50 border-b border-slate-100">
                <div class="flex items-center gap-4 mb-6">
                    <span class="w-12 h-12 rounded-full shadow-sm" style="background-color: ${parti.couleur}"></span>
                    <div>
                        <h3 class="text-2xl font-black text-slate-900">${parti.sigle}</h3>
                        <div class="text-slate-500 font-medium">${parti.nom}</div>
                    </div>
                </div>
